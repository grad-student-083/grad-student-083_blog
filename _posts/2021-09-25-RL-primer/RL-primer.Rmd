---
title: "RL 00: Reinforcement learning primer"
header-includes:
  - \usepackage{float}
...
$$\begin{aligned} 
Q^\pi(\mathbf{x}, a) &= \sum_{\mathbf{x}'}p(\mathbf{x}, a, \mathbf{x}') \left[ r(\mathbf{x}, a) + \gamma V^\pi(\mathbf{x}')\right]\\
&= \sum_{\mathbf{x}'}p(\mathbf{x}, a, \mathbf{x}') \left[ r(\mathbf{x}, a) + \gamma \sum_{a'} \pi(a'|\mathbf{x}') Q^\pi(\mathbf{x}', a') \right]
\end{aligned}$$
